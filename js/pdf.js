...
async function toggleAutoSend(storeId) {
  ...
  sbUpsert("store_settings",{store_id:storeId,auto_send_enabled:!!sd.autoSend}).catch(function(e){console.error("[auto-send]",e);});
  await saveStoreSettings(storeId);
  ...
  function deptSection(d, rdW, rdA) {
    ...
    const kpiCells=dKpis.map(k=>{
      ...
      return `<td style="padding:8px;text-align:center;background:#faf9f6;width:25%">
        ...
        <div style="font-size:9px;color:${GR}">MÃ¥l: ${k.mÃ¥l}%</div>
      </td>`;
    }).join('');
    ...
    return `<div style="margin-bottom:16px;border:1px solid ${BEIGE};border-radius:6px;overflow:hidden">
      <div style="background:${BLUE};color:#fff;padding:7px 12px;font-weight:700;font-size:12px">${d.name} â avd.${d.code}</div>
