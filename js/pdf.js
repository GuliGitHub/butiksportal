...
async function loadPodcastPrompt(){const{data}=await sb.from('podcast_settings').select('prompt_template').eq('id',1).maybeSingle();const ta=document.getElementById('podcast-prompt-inp');if(ta&&data)ta.value=data.prompt_template||'';}async function savePodcastPrompt(){const btn=document.getElementById('podcast-prompt-save-btn'),st=document.getElementById('podcast-prompt-status'),ta=document.getElementById('podcast-prompt-inp');if(!ta)return;if(btn){btn.disabled=true;btn.textContent='Sparar…';}try{const{error}=await sb.from('podcast_settings').upsert({id:1,prompt_template:ta.value,updated_at:new Date().toISOString()},{onConflict:'id'});if(error)throw new Error(error.message);if(st){st.textContent='✓ Sparad';st.style.color='var(--ö-green)';}setTimeout(()=>{if(st)st.textContent='';},3000);}catch(e){if(st){st.textContent='✗ '+e.message;st.style.color='red';}}finally{if(btn){btn.disabled=false;btn.textContent='Spara mall';}}}const m=document.querySelector('input[name=pmode]:checked')?.value;document.getElementById('pdf-psel').style.display=(m==='period'||m==='both')?'block':'none';}
